...
###                                                                                              ###
### Jennifer Meneghin   modicficated by Eduardo Martinez                                         ###
### July 23, 2009       Januray 11, 2018                                                         ###
###                                                                                              ###
...
### September 21, 2010                                                                           ###
### This script now produce an outputfile with the name	input file plus txt extenssion           ###
### Janury 18, 2018                        						         ###
####################################################################################################
#---------------------------------------------------------------------------------------------------------------------------
...
#The main event
#---------------------------------------------------------------------------------------------------------------------------
print OUT "ID\t% GCContent\tTotal Count\tG Count\tC Count\tA Count\tT Count\tN Count\t% NContent\n";
$seq = "";
while (<IN>) {
    chomp;
    if (/^>/) {
...
sub process_it {
	...
	if ($totalcount > 0) {
	$ncontent = (100 * $ncount) / $totalcount;
    }
        else {
	$gccontent = 0;
    }
    print OUT "$gccontent\t$totalcount\t$gcount\t$ccount\t$acount\t$tcount\t$ncount\t$ncontent\n";
    $seq = "";
}
